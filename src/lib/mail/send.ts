...
export async function sendMail(mail: Mail): Promise<SendResult> {
  ...
  if (provider === "none") return { ok: false, provider, error: "MAIL_PROVIDER is not configured" };

  try {
    if (provider === "ses") {
      const region = process.env.AWS_REGION?.trim();
      if (!region || !from) return { ok: false, provider, error: "AWS_REGION and MAIL_FROM are required for SES" };
    ...
    const res = await fetch(`${url.replace(/\/$/, "")}/api/v1/send`, {
      ...
      body: JSON.stringify({
        From: match ? { Name: match[1].trim(), Email: match[2] } : { Email: sender },
        To: [{ Email: mail.to }], Subject: mail.subject, Text: mail.text, HTML: mail.html,
      }),
    });
    if (!res.ok) return { ok: false, provider, error: `Mailpit returned ${res.status}` };
    return { ok: true, provider };
  } catch (error) {
    return { ok: false, provider, error: error instanceof Error ? error.message : "Email delivery failed" };
  }
}
