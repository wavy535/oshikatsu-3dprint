...
export default async function Step1Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const objects = [...(asset?.work_asset_objects ?? [])].sort(
    (a, b) => a.object_index - b.object_index
  );
      ...
      {asset && (
        <>
            ...
            <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                [
                  "原寸の見積り",
                  base
                    ? `${base.est_filament_grams ?? "—"} g ／ ${base.est_print_hours ?? "—"} 時間`
                    : "—",
                ],
              ].map(([label, value]) => (
          ...
          <div className="flex items-center gap-3">
            ...
            <Button asChild className="ml-auto" disabled={failed}>
