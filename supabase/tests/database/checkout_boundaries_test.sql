...
select ok(not has_function_privilege('anon', 'public.place_demo_order(uuid, uuid, text)', 'execute'), 'anonymous users cannot place demo orders');
select ok(not has_function_privilege('service_role', 'public.place_demo_order(uuid, uuid, text)', 'execute'), 'demo checkout requires a user session');
select ok(not has_function_privilege('authenticated', 'public.begin_order_checkout(uuid)', 'execute'), 'users cannot start retired Stripe checkout');
select ok(not has_function_privilege('service_role', 'public.apply_stripe_checkout(uuid, text, integer, text, boolean, text)', 'execute'), 'retired Stripe webhook RPC is not exposed');
select ok(not has_function_privilege('authenticated', 'public.confirm_order_payment(uuid, text)', 'execute'), 'raw payment confirmation stays internal');
select ok(not has_function_privilege('authenticated', 'public.place_order(uuid, text)', 'execute'), 'users cannot bypass atomic demo checkout');
select ok(not has_table_privilege('authenticated', 'public.orders', 'insert'), 'users cannot fabricate orders by direct insert');
...
select is((select sum(quantity) from public.cart_items ci join public.carts c on c.id = ci.cart_id where c.user_id = auth.uid()), 2::bigint, 'failed checkout preserves cart');
...
select is((select count(*) from public.cart_items ci join public.carts c on c.id = ci.cart_id where c.user_id = auth.uid()), 0::bigint, 'successful checkout clears cart');
